...
public class StockStoryController {
    ...
    public ResponseEntity<List<StockStoryDto>> readStockStoriesOfCertainStock(
            @AuthenticationPrincipal UserDetailsDto userDetails,
            @RequestParam("stock-code") String stockCode
    ){
        return ResponseEntity.ok()
                .body(stockStoryService.readStockStoryOfCertainStock(userDetails.getUserId(), stockCode));
    }

    @GetMapping("/{id}")
    public ResponseEntity<StockStoryDto> readStockStoryById(
    ...
    @PostMapping
    public ResponseEntity<CreateStatusDto> createStockStory(
            @AuthenticationPrincipal UserDetailsDto userDetails,
            @RequestBody StockStoryDto createStockStoryDto
    ){
        CreateStatusDto createStatusDto = stockStoryService.createStockStory(userDetails.getUserId(), createStockStoryDto);
